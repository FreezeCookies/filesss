...
def check_server():
    try:
        ...
        if 'status' in data and data['status'] == 'live':
            ...
            update_link = "https://zmatrixtool.x10.mx/"
            ...
            if reply == QMessageBox.Ok:
                webbrowser.open(update_link)  # mở link tải trực tiếp
            raise SystemExit("Cập nhật tool mới tại: https://zmatrixtool.x10.mx/ ")

    except requests.exceptions.RequestException as e:
        QMessageBox.critical(None, "Server Error", f"Lỗi kết nối: {str(e)}\nVui lòng kiểm tra mạng.")
        raise SystemExit(f"Server connection error: {str(e)}")

    except ValueError:
        QMessageBox.critical(None, "Server Error", "Phản hồi không hợp lệ từ server. Vui lòng thử lại sau.")
